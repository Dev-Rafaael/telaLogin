class Login{
  ...
  static config={
    cor:null,
    img:null,
   endpoint:null //"  https://curso-javascript.rafael1327ff.repl.co/"
  ...
  static login=(callback_ok,callback_naook,config=null)=>{
    ...
    sessionStorage.setItem("acessologado","")
      ...
      ".elementosLogin{display: flex; justify-content: flex-start;align-items: flex-start; flex-direction: column;width: 50%;background-color: #eee;  padding: 10px;border-radius: 10px 0px 0px 10px;}"+
      ...
      `.botoesLogin button{cursor: pointer; background-color:#${this.config.cor};color: #fff;border-radius: 5px;padding: 10px;width: 100px;}`
        ...
        styleEstilo.setAttribute("rel","styleheet")
...
campoLoginUserName.appendChild(labelUserName) 
   ...
   botoesLogin.appendChild(btn_login)
    ...
    imglogoLogin.setAttribute("title","Jack sparrow")
...
}t: center; align-items: center; width: 100%;height:100vh; position: absolute; top: 0px; left: 0px; background-color: rgba(0, 0, 0, 0.75); box-sizing: border-box; }"+
      ".baseLogin{ display: flex;justify-content: center; align-items: stretch;  width: 50%;  box-sizing: inherit; }"+
      ".elementosLogin{display: flex; justify-content: flex-start;align-items: flex-start; flex-direction: column;width: 50%;background-color: #eee;  padding: 10px;border-radius: 10px 0px 0px 10px;}"+
      ".logoLogin{ display: flex;justify-content: center;align-items: center;width: 50%;background-color: #bbb;padding: 10px;border-radius: 0px 10px 10px 0px;box-sizing: inherit;}"+
      ".logoLogin img{ width: 90%;box-sizing: inherit;}"+
...
campoLoginUserName.appendChild(labelUserName) 
  ...
  static verificaLogin=()=>{
    ...
    const endpoint=`https://curso-javascript.rafael1327ff.repl.co/?matricula=${mat}&senha=${pas}`
        fetch(endpoint)
        .then(res=>res.json())
        .then(res=>{
          if (res) {
